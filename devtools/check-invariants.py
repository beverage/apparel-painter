...
Ported from shift-change's gate of the same name (2026-09-03, at this mod's
second release). Exits non-zero after reporting every failure it found.

Each check exists because something broke that way, or because the failure
is silent in game — which is worse, since the symptom then arrives as a
player's screenshot rather than a red line.
"""
...
KEYED = os.path.join(ROOT, "Languages", "English", "Keyed", "ApparelPainter.xml")
...
#: Keys we use that VANILLA defines. Ours are all ApparelPainter_*; anything
#: else here is deliberate reuse and must be justified in this list.
#: Dialog_StandColorPicker subclasses the base game's colour dialog and
#: borrows its window title and its two readback labels rather than
#: re-translating strings the player already knows.
VANILLA_KEYS = {
    "ChooseAColor",
    "CurrentColor",
    "OldColor",
...
# touches it. Both compile clean. AGENTS.md states the rule; this enforces it.
...
#
# THREE idioms, not one. shift-change reaches every key through .Translate(),
# but this mod also hands bare keys to TipRegionByKey (the tab's swatch and
# dropper tooltips) and assigns one to ITab.labelKey. A port that only
# matched .Translate() would have called 6 live keys dead and told you to
# delete them.
KEY_PATTERNS = [
    re.compile(r'"([A-Za-z0-9_.]+)"\s*\.Translate(?:Simple)?\s*\('),
    re.compile(r'TipRegionByKey\s*\([^,]+,\s*"([A-Za-z0-9_.]+)"'),
    re.compile(r'labelKey\s*=\s*"([A-Za-z0-9_.]+)"'),
]
...
# name: the comp never attaches, the mod loads without error, and nothing
# happens. This mod ships NO defs today — the tab is injected at runtime
# precisely to avoid the shared-def commons — so this is insurance against
# the first one, not a live check.
def check_xml_bindings():
    ...
    binding = re.compile(
        r'(?:Class\s*=\s*"ApparelPainter\.([A-Za-z0-9_]+)"'
        r'|<[A-Za-z0-9_]*[Cc]lass>\s*ApparelPainter\.([A-Za-z0-9_]+)\s*</)'
    )
        ...
        for name in names:
            ...
            for match in binding.finditer(text):
                ...
                if named not in declared:
                    line = text.count("\n", 0, match.start()) + 1
                    fail("bindings", "%s:%d names ApparelPainter.%s — no such "
                         "type in Source/" % (rel(path), line, named))
...
# This repository is public. The backlog and decision log that drive it are
# not, and their identifiers mean nothing to a reader here — a bare "as the
# tracker decided" reference in a source comment points at a document nobody
# outside can open, and leaks how the work is organised into a mod's shipped
# source. Write the REASON in the comment instead; the tracking item is where
# the argument lives, not what a stranger reading the code needs.
#
...
# not exactly <Patch>, with one log line as the only symptom. This mod ships
# no Patches/ today (deliberately — see AGENTS.md on the shared-def commons),
# so this guards the first one.
